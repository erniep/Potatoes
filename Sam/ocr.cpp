...
/* Notes
04.13.2015  - Characters not working - " Blue_^, Red_Z, Red_!, Blue_( " (Z thinks it has a space in front) 
            - TODO-1: Try splitting channels and manipulating induvidual channels
            - TODO-2: On tesseract output stream, find first "non-space" character to eliminate problem with Z
            - TODO-3: Test saturation(100) effect on blue walls, effect on Red walls is good.
*/
...
int main(int argc, char **argv)
{
	...
	Camera.set(CV_CAP_PROP_SATURATION, 100);		// Sets Image height with defined macro
	...
	while(1) {
		...
		else if(wallColor == 'R') tess.SetVariable("tessedit_char_whitelist", "ABCDEFGHIJKLMNOPQRSTUVWXYZ!@#$%^&*()");
		...
		if (out[0] == 'A') out = testForCaret(thresh);
...
char getWallColor(cv::Mat myHSV){
		...
		/* Find peak closest to right end */
