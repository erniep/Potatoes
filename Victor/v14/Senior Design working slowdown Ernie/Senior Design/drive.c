...
void Robot_drive_task(void)
{
	...
	while(TRUE)
	{
		...
		case DRIVESTATE_IDLE:																// Reset ramp countesr
			...
			//test++;
			if(test==0) fw_motors(85, 1);
			if(test==1) rv_motors(85, 1);
			test^=1;
			//test++;
			...
			break;
		case DRIVESTATE_FWD:
			Detect_Front(lightsnsr_val[4], front_reads, num_iterations, num_moves, &slow_down_flag);	// Is an intersection coming up? is so, cut the DC down
			...
			if(dutycycle < (ramp_up_counter)) duty = dutycycle;								// ramp up the duty cycle
			else duty = (++ramp_up_counter);
			fwd_pid(u, u2, duty);
			break;
		case DRIVESTATE_REV:
			Detect_Front(lightsnsr_val[5], front_reads, num_iterations, num_moves, &slow_down_flag);	// Is an intersection coming up? is so, cut the DC down
			Detect_Adjacent(lightsnsr_val, side_reads, 2, 3, &num_iterations);					// Are we at an intersection yet?
			Check_Iterations(&u, &u2, &num_iterations, num_moves);								// Check if we need to stop
			u = PIDController(error[0], u);														// Controller, error[0] is a pointer
			u2 = PIDController(error[1], u2);													// error[1] is a pointer
		...
		case FRONTLINEDETECT_BLACK:
			if(val)																			// Are sensors picking up a black?
			{
				...
				if(reads[BLACKS] > ((reads[NULLS] * LINEDETECT_K_NUM_3) / LINEDETECT_K_DEN_3))	// Are we at our threshold?
				{
					...
					{
						if(((moves - iterations) == 1) & (!(*flag))) front_detect_state = FRONTLINEDETECT_SLOWDOWN;
