...
void Robot_drive_task(void)
{
	...
	int8_t e0[3];												// Error arrays
	int8_t e1[3];
	int8_t e2[3];
	int8_t e3[3];
	int32_t u;													// Control efforts
	int32_t u2;
	pid_terms_calc(KP, KI, KD, F_SAMP, pid_coeff);				// Find PID controller coefficients

	rv_motors(80,1);
	//fw_motors(50, 1); //Test :)
	//servo_tangent();

	clearReadCNT(side_reads);									// set counts equal to zero
	while(TRUE)
	{
		// Pend on semaphore - Light Sensor values updated
		Semaphore_pend(Sema_lightsense, BIOS_WAIT_FOREVER);
		error_calc(lightsnsr_val[0], e0);						// Update Errors
		error_calc(lightsnsr_val[1], e1);
		error_calc(lightsnsr_val[2], e2);
		error_calc(lightsnsr_val[3], e3);
		...
		case DRIVESTATE_FWD:
			...
			u = findu(e0, u, pid_coeff);
			u2 = findu(e1, u2, pid_coeff);
...
void error_calc(uint32_t lightsnsr, int8_t * error)
{
	...
	case 0x1:
		...
		break;
	case 0x0:
		if((error[1] > 0) & (error[2] > 0)) error[0] = 7;		// Check last 2 errors
		else if(error[1] > 0) error[0] = 5;						// Check prev error
		if((error[1] < 0) & (error[2] < 0)) error[0] = -7;		// Check last 2 errors if negative
		else if(error[1] < 0) error[0] = 5;  					// Check last error if ngative
		if((error[1] == 0) & (error[2] ==0)) error[0] = 0;		// indeterminate state
		break;
	}
}
