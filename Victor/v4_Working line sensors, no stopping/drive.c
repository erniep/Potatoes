...
void Robot_drive_task(void)
{
	...
	uint8_t side_reads[2];										// 0th element is null reads, and 1st element is non-zero reads
	uint8_t num_iterations = 0;

	tl_motors(60, 15); //Test :)
	while(TRUE)
	{
...
}void error_calc_rev(uint32_t lightsnsr, int8_t * error)
{
	error[2] = error[1];
	error[1] = error[0];
	switch(lightsnsr)
...
*/
void linedetection(uint32_t * lightsense_vals, uint8_t * reads, uint8_t fwd_index, uint8_t rev_index, uint8_t * iterations)
{
	...
	case LINEDETECT_BLACK_0:
		if((lightsense_vals[fwd_index]) & (lightsense_vals[rev_index])) reads[1]++;
		else linedetect_state = LINEDETECT_NULLS_1;
		break;
	case LINEDETECT_NULLS_1:
