...
void Robot_lightsnsr_task(void)
{
	...
	Period = ((Clock * lightsnsr_prd[0]) / fixedpoint_microsec_coeff);									// set periods
	TimerLoadSet(TIMER2_BASE, TIMER_B, (uint16_t)Period -1);
	Period = ((Clock * lightsnsr_prd[1]) / fixedpoint_microsec_coeff);									// set periods
	TimerLoadSet(TIMER3_BASE, TIMER_A, (uint16_t)Period -1);
	Period = ((Clock * lightsnsr_prd[2]) / fixedpoint_microsec_coeff);									// set periods
	TimerLoadSet(TIMER4_BASE, TIMER_A, (uint16_t)Period -1);
	Period = ((Clock * lightsnsr_prd[3]) / fixedpoint_microsec_coeff);									// set periods
...
void timer3A_ISR(void)
{
	...
	/*uint8_t bit1 = (lightsnsr_val[1] & 0x2)>>1; //1   //1 //
	uint8_t bit0 = (lightsnsr_val[1] & 0x4)>>2; //0  //2 //
	uint8_t bit2 = (lightsnsr_val[1] & 0x1);   //2  //0 //2
	lightsnsr_val[1] = bit0 | (bit1<<1) | (bit2>>1);
	*/
	lightsnsr_flag |= 0x2;
	if(lightsnsr_flag == 0xF)
	{
