...
int sendMessage(int serialHandle, char address, char cmd[3], char *args) {
	cout << "(serialComm.sendMessage) sendMessage started" << endl;
	...
	*(tempMessage+2)= 0;
	...
	cout << "(serialComm.sendMessage) cmd: " << cmd << "|" << endl;
	cout << "(serialComm.sendMessage) tempCMD: " << tempCMD << "|" << endl;
	cout << "(serialComm.sendMessage) ARGSIZE: " << argsSize << endl;
	if (argsSize != 0) strcat(finalMessage, args);
	strcat(finalMessage,"011");	
	cout << "(serialComm.sendMessage) Message: " << finalMessage << "|" << endl;
	...
	cout << "(serialComm.sendMessage) sendMessage completed" << endl;
	return 0;
}
...
bool waitAck(int serialHandle, char address) {
	cout << "(serialComm.waitAck) waitAck started" << endl;
	char single;
	char message[100];
			while (1) {
