...
{
	int temp = (GPIOPinRead(LIGHTSNSR1_BASE, LIGHTSNSR1) >> 2) & 0x07;
	int toOut = 0;
	toOut = ((temp << 2) & 0x04) | (temp & 0x02) | ((temp >> 2) & 0x01);
	TimerIntClear(TIMER2_BASE, TIMER_TIMB_TIMEOUT);								// Clear interrupt
	lightsnsr_val[0] = toOut;									// Read light sensors and update global vars
...
{
	int temp = (GPIOPinRead(LIGHTSNSR2_BASE, LIGHTSNSR2) >> 5) & 0x07;
	int toOut = 0;
	toOut = ((temp << 2) & 0x04) | (temp & 0x02) | ((temp >> 2) & 0x01);
	TimerIntClear(TIMER3_BASE, TIMER_TIMA_TIMEOUT);								// Clear interrupt
	lightsnsr_val[1] = temp;									// Read light sensors and update global vars
	lightsnsr_flag |= 0x2;
	if(lightsnsr_flag == 0xF)
	{
