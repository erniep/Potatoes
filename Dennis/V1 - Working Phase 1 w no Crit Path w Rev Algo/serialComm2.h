...
int getMessage(int serialHandle, char _address, char *_data) {
	...
	while(1) {
		...
		if (letter == 3) {	//ctrl+c
			cout << "(serialComm2.sendMessage) End" << endl;
		...
		if (letter == '~') {
			...
			length = atoi(text);
			cout << "(serialComm2.sendMessage) Length: " << length << endl;
			...
			*address = *text;
			...
			cout << "(serialComm2.sendMessage) Address: " << address << endl;
			...
			cout << "(serialComm2.sendMessage) CMD: " << cmd << endl;

			if (argLength > 0) {
					getCharacters(serialHandle,args,argLength);	//address
					calcCS += getChecksum(ptr);
					cout << "(serialComm2.sendMessage) Args: " << address << endl;
			}

			getCharacters(serialHandle,ptr,3);	//address
			checksum = atoi(text);
			cout << "(serialComm2.sendMessage) Checksum: " << checksum << endl;

			cout << "(serialComm2.sendMessage) calcCS: " << calcCS << endl;	//Calculated Checksum includes Address, Command, and Args
