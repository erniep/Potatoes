...
public: 
	...
	bool callUltras(int* wallReadings); //Function to call the wall sensors. Updates the wallReadings array (Left, midddle, and right correspond to indexes 0, 1, and 2). This function assumes that the direction not covered by the wall sensors has no wall. 
...
bool naviSyst::callUltras(int* wallReadings){ //Calls the ultrasonic distance sensors and modifies the wallReadings array to reflect the readings. Left, midddle, and right correspond to indexes 0, 1, and 2
...
void naviSyst::callWallSensorsSim(){ //Simulates sensor readings for a given map defined by the boolean array "maze"
	...
	//Set the status of each wall
	
	c->setWall('E',maze[((n-1)*4) + 1]);
	c->setWall('S',maze[((n-1)*4) + 2]);
	c->setWall('W',maze[((n-1)*4) + 3]);

	//Also set the status of walls in the adjacent cells
	// As of 2/26/2015, this part is incomplete
	//
	//unitCell* northAdj = activeMap->getCell();
	//unitCell* southAdj = activeMap->getCell();
	//unitCell* eastAdj = activeMap->getCell();
	//unitCell* westAdj = activeMap->getCell();	
	//
